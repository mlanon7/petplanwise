...
## Page inventory (~331 built pages as of 2026-06-14)

The site grew from a handful of calculators to ~331 HTML pages via **programmatic-SEO dimensions**. Understand these before adding pages:
...
| **Guides** | `/guides/<topic>/` | 63 | one-shot generators (e.g. `add-condition-guides.js`). Incl. 3 pages added 2026-05/06: `pet-sedation-anesthesia-cost` + `pet-surgery-cost` (pillars on GSC-gap clusters) and `vet-costs-by-state` (50-state data asset, `Dataset` schema + "cite this" block, built by `gen-vet-costs-by-state.js`). |
| **State hubs** | `/states/<state>-pet-cost/` | 25 | programmatic |
| **Calculators** | `/dog-cost-calculator/`, `/cat-cost-calculator/`, `/vet-bill-calculator/`, `/emergency-vet-cost-calculator/`, `/pet-insurance-vs-savings/` | 5 | hand-authored. The first 3 carry an "embed this free" CTA → `/embed/` (`add-embed-cta-to-calculators.js`). |
| **Interactive tools** | `/compare/`, `/find-my-breed/` | 2 | **self-contained** (inline style+script) |
| **Embed** | `/embed/` (landing, **indexable**) + `/embed/cost-calculator/` (widget, `noindex,follow`) | 2 | Link-building asset: the copy-paste snippet bundles a real attribution backlink. As of 2026-06-14 the widget is **no longer robots-blocked** and the landing is `index,follow` + in `sitemap-core`. |
...
**The 8 legacy cat breeds** (bengal, british-shorthair, maine-coon, persian, ragdoll, scottish-fold, siamese, sphynx) use the `<slug>-cat-cost/` directory pattern. Everything else uses `<slug>-cost/`. Helper map: `CAT_LEGACY_DASH_CAT` in `/compare/index.html` and `/find-my-breed/index.html`; generators use `breedDir()` in `add-related-breeds.js`. **2026-06 fix:** related-breed cards had pointed at the wrong `<slug>-cost/` for 6 of these (19 internal links → 404). Fixed in the generator, and the 6 wrong URLs now 301-redirect to `-cat-cost/` in `vercel.json`.

### Off-page assets & infrastructure (not pages, but tracked in the repo)

- **`/brand/`** — `petplanwise-avatar-1000.png` + `petplanwise-cover-pets-1600x900.png`, hosted so platform profile setups (Pinterest, Qwoted) can fetch them by URL instead of a local file picker. Generated by `gen-brand-images.js` / `gen-cover-pets.js`.
- **`/pins/`** — the 6 Pinterest pin PNGs (`gen-pinterest-pins.js`), hosted so Pinterest's "Save from URL" / a platform's "Web Address" uploader can pull them (the autonomous path; the OS file picker can't be driven). Pins live on the PetPlanWise Pinterest account.
- **IndexNow** — `scripts/ping-indexnow.js` submits all sitemap URLs to Bing + Yandex; key file is `<key>.txt` at site root, key passed via `INDEXNOW_KEY` env. Run after a ship to accelerate re-crawl. Operational since 2026-05.
- **Email & DNS** — domain email (ImprovMX forwarding + Gmail "Send mail as" + SPF/DKIM/DMARC) is documented in [`docs/email-and-dns.md`](docs/email-and-dns.md). No secrets in repo.
- **Growth / outreach** (local + memory, NOT in this repo): the prospect list, pitch templates, and send logs live in `_research/outreach-playbook.md` (gitignored). Platform-image upload workarounds (Pinterest/Qwoted "from URL") are in the auto-memory file `pinterest-save-from-url-flow.md`. Qwoted source profile is live; data-asset + embed-widget outreach batches are drafted/sent from `martin@petplanwise.com`.
...
| SHA | Summary |
|---|---|
| `a17c0402` | Make the embeddable calculator earn links: `/embed/` indexable, robots unblocked, snippet bundles attribution backlink, CTA on calc pages |
| `622f05d5`/`bf4a464d` | Fix 19 internal links to 404 (legacy cat breeds) + 6 `vercel.json` 301 redirects |
| `86115894` | Host brand avatar + cover at `/brand/` for platform profile setup |
| `752b5b23`/`5bc855e6` | Host 6 Pinterest pins at `/pins/`; swap puppy pin to a real puppy photo |
| `f42ca8f9` | Add `vet-costs-by-state` data asset (linkable 50-state table, `Dataset` schema) |
| `e80fcd74`/`ad18aaba` | Add `pet-sedation-anesthesia-cost` + `pet-surgery-cost` pillar pages (GSC head-term gaps) |
...
Last reviewed: 2026-06-14 (added the June growth sprint: 3 guide pages incl. the `vet-costs-by-state` data asset, the `/embed/` link-building overhaul, `/brand/` + `/pins/` hosted assets, the legacy-cat-breed 404 fix + redirects, IndexNow notes, and the new `docs/email-and-dns.md`).
