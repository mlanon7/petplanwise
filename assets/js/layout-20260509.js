...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    /* Last-updated stamp + reviewer credit block under H1.
       Reviewer details come from /assets/data/csv/reviewer.csv via PETCOST_DATA. */
    ...
    if (!isLegal) {
      ...
      /* Inject reviewer block once data has loaded — only on guide/breed/state pages */
      var isContentPage = /\/(guides|breeds|states|vet-costs)\//.test(path) || path === "/";
      if (isContentPage && !document.querySelector(".reviewer-block-dynamic")) {
        function injectReviewer() {
          var D = window.PETCOST_DATA || {};
          var R = D.reviewer || null;
          if (!R) return;
          var stamp = document.querySelector(".last-updated-stamp");
          if (!stamp || document.querySelector(".reviewer-block-dynamic")) return;
          var block = document.createElement("div");
          block.className = "reviewer-block reviewer-block-dynamic";
          block.innerHTML =
            '<img class="reviewer-photo" src="' + (R.photo_url || "/assets/images/reviewer-placeholder.svg") + '" alt="" width="44" height="44" />' +
            '<div class="reviewer-meta">' +
              '<div class="reviewer-name">' + (R.name || "PetPlanWise Editorial") + '</div>' +
        ...
        if (window.PETCOST_DATA && typeof window.PETCOST_DATA.ready === "function") {
          window.PETCOST_DATA.ready().then(injectReviewer).catch(injectReviewer);
        } else {
          injectReviewer();
        }
      }
